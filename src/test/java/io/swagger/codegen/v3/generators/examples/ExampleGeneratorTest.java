...
public class ExampleGeneratorTest {
    ...
    @Test
    public void testExampleWithRecursiveNodes2() throws Exception {
        final String content = FileUtils.readFileToString(new File(getClass().getClassLoader().getResource("3_0_0/recursiveExampleGeneration.json").getFile()));
        final ParseOptions options = new ParseOptions();
        options.setResolve(true);
        options.setFlatten(true);
        final SwaggerParseResult result = new OpenAPIParser().readContents(content, null, options);
    ...
    @Test
    public void testExampleWithAnotherCycleRefsPoint() throws Exception {
        final String content = FileUtils.readFileToString(new File(getClass().getClassLoader().getResource("3_0_0/recursiveExampleGeneration3.json").getFile()));
        final ParseOptions options = new ParseOptions();
        options.setResolve(true);
        options.setFlatten(true);
        final SwaggerParseResult result = new OpenAPIParser().readContents(content, null, options);
        ...
        final Schema documentBasicSchema = openAPI.getComponents().getSchemas().get("Document");
        final ExampleGenerator exampleGenerator = new ExampleGenerator(openAPI);

        final List<Map<String, String>> exampleList = exampleGenerator.generate(null, null, documentBasicSchema);
        Assert.assertEquals(exampleList.size(), 1);
        final Map<String, String> example = exampleList.get(0);
        Assert.assertEquals(example.get("contentType"), "application/json");    
        String children = "\"children\" : [ {\n    \"name\" : \"name\",\n";
        Assert.assertTrue(example.get("example").contains(children));
